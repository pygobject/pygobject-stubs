...
def _type_to_python(
    ...
    if tag == tags.ARRAY:
        ...
        return f"{stub.get_import('typing', 'Sequence')}[{t}]"
    ...
    if tag == tags.GTYPE:
        return (
            f"{stub.get_import('typing', 'Type')}[{stub.get_import('typing', 'Any')}]"
        )
    ...
    if tag == tags.INTERFACE:
        ...
        if isinstance(interface, GI.CallbackInfo):
            ...
            if len(return_args) == 1:
                ...
                return_type = (
                    f"{stub.get_import('typing', 'Tuple')}[{', '.join(return_args)}]"
                )

            # FIXME, how to express Callable with variable arguments?
            if (len(names) > 0 and names[-1].startswith("*")) or varargs:
                return f"{stub.get_import('typing', 'Callable')}[..., {return_type}]"
            else:
                return f"{stub.get_import('typing', 'Callable')}[[{', '.join(args)}], {return_type}]"
        else:
            ...
            if namespace == "GObject" and name == "Closure":
                return f"{stub.get_import('typing', 'Callable')}[..., {stub.get_import('typing', 'Any')}]"
...
def _build_function_info(
    ...
    if return_signature:
        ...
        return_type = f"{stub.get_import('typing', 'Tuple')}[{', '.join(return_args)}]"
    else:
...
def _wrapped_strip_boolean_result(
    ...
    if fail_ret is None:
        ...
        if type(fail_ret) is tuple:
            if len(fail_ret) > 0:
                return_signature = f"({return_signature} | {stub.get_import('typing', 'Tuple')}{str(fail_ret).replace('(', '[').replace(')', ']')})"
            else:
                return_signature = (
                    f"({return_signature} | {stub.get_import('typing', 'Tuple')}[()])"
                )
        else:
...
class Stub:
    ...
    def get_import(self, module: str, name: str | None = None) -> str:
        if (_import := self.__get_import(module, None)) is not None:
    ...
    def __replace_typing(self, match: re.Match[str]) -> str:
        return (
            f"{self.get_import('typing', match.group('name'))}{match.group('suffix')}"
        )
    ...
    def __replace_free_typing(self, match: re.Match[str]) -> str:
        ...
        symbol = (
            ...
            if name == "Any"
            else self.get_import("typing_extensions", "Self")
        )
    ...
    def format_annotation(self, annotation: typing.Any) -> str:
        try:
