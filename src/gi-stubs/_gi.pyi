...
class GInterface(ObjectProtocol, Protocol): ...

@type_check_only
class GProps(Protocol): ...

# The following two protocols are used to avoid duplication of the GObject methods in
# GObject, GInterface, and GObject.Object. Because GInterface can only be used by mixing
# it in with an Object but is not itself a subclass of Object, we use these protocols to
# aid users who (rightly) expect the GObject methods when writing GInterface methods. The
# advantage of this solution is that `issubclass(GInterface, GObject.Object)` is still
# False both at runtime and during type-checking and maintains the correct MRO for both
# GInterface and Object.
@type_check_only
class GObjectProtocol(Protocol):
    @property
    def props(self) -> GProps: ...
    def bind_property(
        self,
        source_property: str,
        target: GObject,
...
@type_check_only
class ObjectProtocol(GObjectProtocol, Protocol):
    @property
    def g_type_instance(self) -> _GObject.TypeInstance: ...
    @property
    ...
    def connect_data(
        ...
        *data: object,
        connect_flags: _GObject.ConnectFlags = _GObject.ConnectFlags.DEFAULT,
    ) -> int: ...
    ...
    def install_properties(self, pspecs: Sequence[_GObject.ParamSpec]) -> None: ...
    def install_property(self, property_id: int, pspec: _GObject.ParamSpec) -> None: ...
    def is_floating(self) -> bool: ...
    def list_properties(self) -> list[_GObject.ParamSpec]: ...
    @classmethod
    def newv(
        cls, object_type: type[Any], parameters: Sequence[_GObject.Parameter]
    ) -> Self: ...
    ...
    # The following are intentionally left untyped because they are
    # fake functions that shouldn't be used by end-users, but also
    # should not cause type checkers to flag an overload error if
    # they are used in subclasses
