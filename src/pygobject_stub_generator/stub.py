    ...
    def get_callable_arguments(
        ...
        if (varargs_info := arguments.varargs_info) is not None:
            args[f"*{self.fix_argument_name(varargs_info.get_name())}"] = (
                f"{self.get_import('typing', 'Unpack')}[{self.get_callback_typevar_tuple()}]"
            )
        ...
        match tag:
            ...
            case GI.TypeTag.ARRAY | GI.TypeTag.GLIST | GI.TypeTag.GSLIST:
                ...
                if tag == GI.TypeTag.ARRAY:
                    if out:
                        ...
                        py_type = f"{self.get_import('collections.abc', 'Sequence')}[{value_type}]"
                else:
                    py_type = f"list[{value_type}]"
            ...
            case GI.TypeTag.INTERFACE:
                ...
                if isinstance(interface_info, GI.CallbackInfo):
                    ...
                    #     ):
                    #         py_type = f"{py_type}[{self.get_import('typing', 'Unpack')}[{self.get_callback_typevar_tuple()}]]"
                    # else:
                    #     # Generate the `Callable[]` annotation for anonymous callbacks
                    #     (callback_args, callback_return_args) = (
                    #         self.get_callable_arguments(interface_info, allow_varargs=varargs_callback)
                    #     )
                    #     callback_return_type = get_return_type(callback_return_args)
                    #
                    #     py_type = f"{self.get_import('collections.abc', 'Callable')}[[{', '.join(callback_args.values())}], {callback_return_type}]"
                    ...
                    if interface_namespace == "GObject" and interface_name == "Value":
                        ...
                        py_type = f"{self.get_import('collections.abc', 'Callable')}[..., {self.get_import('typing', 'Any')}]"
                        ...
                        py_type = f"{self.get_namespace_member('cairo', 'Context')}[{some_surface}]"
                    else:
                        py_type = self.get_namespace_member(
    ...
    def __wrapped_strip_boolean_result(
        ...
        if fail_ret is None:
            ...
            if type(fail_ret) is tuple:
                ...
                return_signature = f"({return_signature} | {self.get_import('typing', 'Literal')}[{fail_ret}])"
        ...
        *,
        ...
        if isinstance(function, GI.FunctionInfo) or isinstance(function, GI.VFuncInfo):
            if in_class is None and function.get_namespace() != self.namespace:
                ...
                return f"{name}: {self.get_final()} = {self.get_namespace_member(function.get_namespace(), f'{function.get_name()}')}\n"
